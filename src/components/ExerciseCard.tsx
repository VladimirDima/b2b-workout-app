...
export function ExerciseCard({ exercise, weekData, setLogs, prevSetLogs, exerciseDone, onSetChange }: ExerciseCardProps) {
  ...
  return (
    <article className={`exercise-card ${exerciseDone ? 'exercise-done' : ''}`}>
      <div className="exercise-header">
        ...
        <div className="exercise-btn-group">
          {exercise.muscleGroups.length > 0 && (
            <MuscleMapButton exerciseName={exercise.name} muscleGroups={exercise.muscleGroups} inline />
          )}
          {exercise.videoUrl && (
            <button
              type="button"
              className="exercise-btn-group-item"
      ...
      <div className="set-logging">
        <div className={`set-log-header ${prevSetLogs ? 'has-prev' : ''}`}>
          <span>Set</span>
          {prevSetLogs && <span className="prev-header">Prev</span>}
          <span>Weight</span>
          <span>Reps</span>
          <span>Done</span>
        </div>
        {Array.from({ length: setCount }, (_, i) => {
          ...
          const prev = prevSetLogs?.[i];
          const prevLabel = prev && (prev.weight || prev.reps)
            ? `${prev.weight || '0'}×${prev.reps || '0'}`
            : undefined;
          return (
            <div key={i} className={`set-row ${log.completed ? 'completed' : ''} ${prevSetLogs ? 'has-prev' : ''}`}>
              <span className="set-num">{i + 1}</span>
              {prevSetLogs && (
                <span className="prev-values" title="Previous week">{prevLabel ?? '—'}</span>
              )}
              <input
                type="text"
                inputMode="decimal"
